...
@Component({
  selector: 'app-estadisticas-enfermero',
  providers: [{ provide: MAT_DATE_FORMATS, useValue: FORMATO_MES }],
  templateUrl: './estadisticas-enfermero.component.html',
  styleUrls: ['./estadisticas-enfermero.component.scss']
})
export class EstadisticasEnfermero implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private servicioEstadisticas: ServicioEstadisticasEnfermero,
  ) {
  ...
  private inicializarGraficosCirculares() {
    ...
    console.log(listadoGraficosCirculares)
  ...
  private inicializarGraficoCircularTodosCentros(opcionesGrafico: any) {
    ...
    return new Chart(this.getCTXDeGrafico(Globales.TIPOS_GRAFICOS.GRAFICO_CIRCULAR, this.datosGraficos.length), {
      ...
      options: opcionesGrafico,
      data: {
        // labels: this.getEtiquetasTodosGraficosCirculares(),
        ...
        datasets: this.datosGraficos.map((centro: any) => {
          return {
            data: listadoIdsTodasPruebas.map((idPrueba: number) => {
              ...
              console.log(centro.nombreCentro+" "+idPrueba+" "+indicePrueba)
  ...
  // private inicializarGraficos(tipoGrafico: any) {
  //   const opcionesGrafico: any = {
  //     scales: {
  //       x: {
  //         display: !(tipoGrafico === Globales.TIPOS_GRAFICOS.GRAFICO_CIRCULAR),
  //         title: {
  //           display: !(tipoGrafico === Globales.TIPOS_GRAFICOS.GRAFICO_CIRCULAR),
  //           text: tipoGrafico === Globales.TIPOS_GRAFICOS.GRAFICO_BARRAS_VERTICALES && this.busquedaAnterior.modoAnual
  //             ? 'Mes'
  //             : 'Día',
  //           color: tipoGrafico === Globales.TIPOS_GRAFICOS.GRAFICO_BARRAS_VERTICALES
  //             ? '#3956ff'
  //             : '#ff5a00',
  //           font: {
  ...
  //   };
  //   const datosGrafico = {
  //     labels: this.getEtiquetas(tipoGrafico),
  //     datasets: this.datosGraficos?.map(
  //       (centro) => {
  //         const color = this.generarColorAleatorio();
  //         return {
  //           data: tipoGrafico === Globales.TIPOS_GRAFICOS.GRAFICO_BARRAS_VERTICALES ? centro.totalCitas : centro.pruebas,
  //           label: centro.nombreCentro,
  //           backgroundColor: tipoGrafico === Globales.TIPOS_GRAFICOS.GRAFICO_CIRCULAR
  //             // ? centro.items?.map(() => this.generarColorAleatorio())
  //             ? centro.totalCitas.map((valor: string, indice: number) => this.listadoColoresMesDia[indice])
  //             : color,
  //           borderColor: undefined,
  //         };
  //       },
  //     ),
  //   };
  //   return new Chart(this.getCTXDeGrafico(tipoGrafico), {
  //     type: this.getConfiguracionTipoGrafico(tipoGrafico),
  //     options: opcionesGrafico,
  //     data: datosGrafico,
  //   });
  // }
  ...
  private getCTXDeGrafico(tipoGrafico: number, indiceGrafico: number) {
    switch (tipoGrafico) {
      case Globales.TIPOS_GRAFICOS.GRAFICO_BARRAS_VERTICALES: {
        console.log(this.CTXGraficoBarrasVerticales.nativeElement.getContext('2d'))
        return this.CTXGraficoBarrasVerticales.nativeElement.getContext('2d');
      }
      case Globales.TIPOS_GRAFICOS.GRAFICO_CIRCULAR: {
        console.log("Indice: "+indiceGrafico)
        console.log(this.CTXGraficosCirculares.get(indiceGrafico).nativeElement.getContext('2d'))
  ...
  public pedirEstadisticas() {
    ...
    if (this.formulario.valid) {
      ...
      this.servicioEstadisticas.getDatosGraficosEstadisticas(
